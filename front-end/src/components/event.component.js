...
export default class Event extends Component {
    ...
    componentDidMount() {
        axios.get(`http://${process.env.REACT_APP_BACKEND_URL}/api/events/${this.props.location.state.id}`,
        {
            headers: {
    ...
    handleSubmit = event => {
        ...
        axios.delete(`http://${process.env.REACT_APP_BACKEND_URL}/api/events/${this.props.location.state.id}`,
            {
                headers:{
                    'Content-Type': 'multipart/form-data',
                    'Authorization': `${this.props.location.state.token}`
